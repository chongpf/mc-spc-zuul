...
public class SpcConfigPropertyController {
    ...
    @GetMapping("/property")
    @ApiOperation(value = "全部配置信息", notes = "git提交后，务必执行method=Post,/actuator/refresh",
    ...
    @ApiOperation(value = "获取指定属性的值", notes = "获取指定属性的值",
            response = ResponseData.class)
    @ApiImplicitParams({
            @ApiImplicitParam(name = "pName", required = true, paramType = "query",
                    dataType = "String")
    })
